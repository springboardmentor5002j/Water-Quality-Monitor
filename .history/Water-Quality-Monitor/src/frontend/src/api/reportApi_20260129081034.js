...
export const getVerifiedReportsByLocation = async (location) => {
  const res = await fetch(
    `http://localhost:8000/stations/verified_reports_by_location?location=${location}`
  );
  return res.json();
};
...
// =================================================
// ✅ WEEK–6 / WEEK–7 : Historical Readings by Location
// =================================================
export async function fetchHistoricalReadings(location, parameter) {
  try {
    const res = await axios.get(
      `${API}/readings/history/location`,
      {
        params: {
          location,
          parameter, // must be "pH", "DO", etc
        },
      }
    );
    return res.data;
