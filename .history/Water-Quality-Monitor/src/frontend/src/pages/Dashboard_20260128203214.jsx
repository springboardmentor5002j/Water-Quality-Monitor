...
// MapUpdater: adjusts the map center
    ...
    <div className="min-h-screen bg-gray-100">
      <div className="max-w-7xl mx-auto grid grid-cols-[220px_1fr] gap-6 p-6">
        ...
        <aside className="bg-white p-4 rounded shadow space-y-3">
          <div className="font-semibold">📍 Map & Data</div>
          <Link to="/report" className="block text-blue-600 hover:underline">
            Submit Report
          </Link>
          <Link to="/reports/my" className="block text-blue-600 hover:underline">
            My Reports
          </Link>
          <div>Stations</div>
    ...
    {stations.length === 0 ? (
      ...
      stations.map((s) => (
        <div key={s.id} className="border p-4 rounded mb-3 bg-gray-50">
          ...
          <p><strong>Created At:</strong>{" "} {new Date(s.created_at).toLocaleString()}</p>
        </div>
      ))
    )}
  </>
    ...
    {stations.length === 0 ? (
      ...
      stations.map((s) => (
        <div key={s.id} className="border p-4 rounded mb-4 bg-gray-50">
          <p className="font-semibold">Station: {s.name} ({s.location})</p>
          {s.latest_readings && s.latest_readings.length > 0 ? (
              ...
              <tbody>
                {s.latest_readings.map((r, idx) => (
                  <tr key={idx}>
                    ...
                    <td className="border p-1">{new Date(r.recorded_at).toLocaleString()}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ) : (
            <p className="text-sm mt-2">No readings available for this station.</p>
          )}
        </div>
      ))
    )}
...
const VerifiedReportsTable = ({ reports }) => (
  <>
    <h3 className="text-lg font-semibold mb-2">Verified Reports</h3>
    {reports.length === 0 ? (
      <div className="p-4 bg-yellow-50 border border-yellow-300 rounded text-yellow-800">
      ...
      reports.map((r) => (
        <div key={r.id} className="border p-3 mb-2 rounded">
          {r.description}
          <br />
          <small>{r.water_source} • {r.reported_by}</small>
        </div>
      ))
    )}
  </>
